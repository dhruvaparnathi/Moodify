...
const Register = () => {
  const { loading, handleRegister, handleVerifyEmail, handleResendOtp } = useAuth();
  ...
  useEffect(() => {
    ...
    gsap.set(".auth-nav-header, .auth-card, .subheading-badge, .auth-equalizer span, .register-form h2, .register-form p.form-subtitle, .input-group, .register-btn, .login-link, .auth-mascot-container", {
      opacity: 0
    });
    ...
    tl.fromTo(".auth-nav-header",
      ...
      .fromTo(".register-form h2, .register-form p.form-subtitle, .input-group, .register-btn, .login-link",
        { y: 15, opacity: 0 },
  ...
  if (loading) {
    return <Loader text={step === "register" ? "Creating your Account..." : "Verifying code..."} />;
  }
  ...
  const handleVerifySubmit = async (e) => {
    ...
    setSuccessMessage("");
    try {
      await handleVerifyEmail({ email: formData.email, otp });
      navigate('/');
    } catch (err) {
      if (err.response && err.response.data) {
        if (err.response.data.errors) {
  ...
  const handleResend = async () => {
    ...
    setSuccessMessage("");
    try {
      const data = await handleResendOtp({ email: formData.email });
      ...
      <div className="auth-card">
        ...
        <div className="auth-mascot-container">
          ...
          <div className="speech-bubble">
            {step === "register" ? "Join the vibe! ⚡" : "Check your inbox! ✉️"}
          </div>
        </div>
        ...
        {step === "register" ? (
          <form className="register-form" onSubmit={handleSubmit}>
            ...
            <p className="form-subtitle">Join Moodify and start your emotion-driven playlist journey.</p>
          ...
          <form className="register-form" onSubmit={handleVerifySubmit}>
            <h2>Verify Email</h2>
            <p className="form-subtitle">We've sent a 6-digit verification code to <strong>{formData.email}</strong>. Please enter it below.</p>
            ...
            <p className="login-link" style={{ marginBottom: "8px" }}>
            ...
            <p className="login-link">
              Need to change details? <span onClick={() => { setStep("register"); setErrors([]); setSuccessMessage(""); }} style={{ color: "#f08ba4", cursor: "pointer", fontWeight: "700", textDecoration: "underline" }}>Go Back</span>
            </p>
          </form>
        )}
      </div>
